...
deny[msg] if {
    ...
    msg := sprintf(
        "CRITICAL: Protected resource %s of type %s is scheduled for %s. " +
        "Use --break-glass if this is an emergency.",
...
deny[msg] if {
    resource := input.resource_changes[_]
    contains(resource.name, "production")
...
warn[msg] if {
    ...
    msg := sprintf(
        "WARNING: Protected resource %s of type %s is scheduled for replacement. " +
